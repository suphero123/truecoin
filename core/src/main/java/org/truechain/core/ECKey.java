...
import org.truechain.crypto.EncryptableItem;
import org.truechain.crypto.EncryptedData;
import org.truechain.crypto.KeyCrypter;
import org.truechain.crypto.KeyCrypterException;
import org.truechain.crypto.LazyECPoint;
import org.truechain.crypto.LinuxSecureRandom;
...
public class ECKey implements EncryptableItem {
    ...
    public static ECKey fromPrivateAndPrecalculatedPublic(byte[] priv, byte[] pub) {
        Utils.checkNotNull(priv);
        Utils.checkNotNull(pub);
    ...
    public ECKey(EncryptedData encryptedPrivateKey, byte[] pubKey, KeyCrypter keyCrypter) {
        ...
        this.keyCrypter = Utils.checkNotNull(keyCrypter);
    ...
    public static ECKey fromEncrypted(EncryptedData encryptedPrivateKey, KeyCrypter crypter, byte[] pubKey) {
        ...
        key.encryptedPrivateKey = Utils.checkNotNull(encryptedPrivateKey);
        key.keyCrypter = Utils.checkNotNull(crypter);
        return key;
    }
    ...
    private static ECKey extractKeyFromASN1(byte[] asn1privkey) {
        ...
        try {
            ...
            DLSequence seq = (DLSequence) decoder.readObject();
            Utils.checkState(decoder.readObject() == null, "Input contains extra bytes");
            decoder.close();

            Utils.checkState(seq.size() == 4, "Input does not appear to be an ASN.1 OpenSSL EC private key");

            Utils.checkState(((ASN1Integer) seq.getObjectAt(0)).getValue().equals(BigInteger.ONE),
            ...
            Utils.checkState(pubkey.getTagNo() == 1, "Input has 'publicKey' with bad tag number");
            byte[] pubbits = ((DERBitString)pubkey.getObject()).getBytes();
            Utils.checkState(pubbits.length == 33 || pubbits.length == 65, "Input has 'publicKey' with invalid length");
            int encoding = pubbits[0] & 0xFF;
            // Only allow compressed(2,3) and uncompressed(4), not infinity(0) or hybrid(6,7)
            Utils.checkState(encoding >= 2 && encoding <= 4, "Input has 'publicKey' with invalid encoding");
    ...
    public static ECKey recoverFromSignature(int recId, ECDSASignature sig, Sha256Hash message, boolean compressed) {
    	Utils.checkState(recId >= 0, "recId must be positive");
    	Utils.checkState(sig.r.signum() >= 0, "r must be positive");
    	Utils.checkState(sig.s.signum() >= 0, "s must be positive");
    ...
    public ECKey decrypt(KeyCrypter keyCrypter, KeyParameter aesKey) throws KeyCrypterException {
        ...
        Utils.checkState(encryptedPrivateKey != null, "This key is not encrypted");
    ...
    public String getPrivateKeyAsHex() {
        return Base16.encode(getPrivKeyBytes());
    }

    public String getPublicKeyAsHex() {
        return Base16.encode(pub.getEncoded());
    }
    ...
    public void formatKeyWithAddress(boolean includePrivateKeys, StringBuilder builder, NetworkParameters params) {
        ...
        builder.append(Base16.encode(getPubKeyHash()));
        if (creationTimeSeconds > 0)
