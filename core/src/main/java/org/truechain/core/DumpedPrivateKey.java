...
public class DumpedPrivateKey extends VersionedChecksummedBytes {
	private static final long serialVersionUID = -2010012353621127529L;
    ...
    private static byte[] encode(byte[] keyBytes, boolean compressed) {
        Utils.checkState(keyBytes.length == 32, "Private keys must be 32 bytes");
        if (!compressed) {
            return keyBytes;
        } else {
    ...
    @Override
    public int hashCode() {
        return Objects.hash(version, compressed, Arrays.hashCode(bytes));
    }
}
